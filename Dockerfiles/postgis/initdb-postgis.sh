...
CREATE ROLE postgis ENCRYPTED PASSWORD 'postgis' LOGIN;
EOSQL
$POSTGRES postgres --single -E <<EOSQL
CREATE DATABASE fixmystreet OWNER postgis ;
EOSQL
$POSTGRES pg_ctl -w start
$POSTGRES psql -d fixmystreet -c 'CREATE EXTENSION postgis;'
$POSTGRES psql -d fixmystreet -c 'CREATE EXTENSION postgis_topology;'
